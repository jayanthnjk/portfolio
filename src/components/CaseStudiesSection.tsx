...
function GatewayArchDiagram() {
  return (
    <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4 overflow-x-auto scrollbar-hide">
      <div className="min-w-[500px] space-y-3">
        {/* Row 1: Ingress Layer */}
        <ArchRow label="Ingress" color="#3b82f6" delay={0}>
          <ArchNode label="Auth & Identity" color="#3b82f6" delay={0.1} />
          <ArchNode label="Rate Limiter (Redis)" color="#3b82f6" delay={0.2} mono />
          <ArchNode label="Concurrency Control" color="#3b82f6" delay={0.3} />
        </ArchRow>

        {/* Row 2: Policy Engine */}
        <ArchRow label="Policy" color="#8b5cf6" delay={0.4}>
          <ArchNode label="Token Budget" color="#8b5cf6" delay={0.5} />
          <ArchNode label="DLP Scanner" color="#8b5cf6" delay={0.55} />
          <ArchNode label="Tool Allowlist" color="#8b5cf6" delay={0.6} />
          <ArchNode label="Model Access" color="#8b5cf6" delay={0.65} />
        </ArchRow>

        {/* Row 3: Routing */}
        <ArchRow label="Router" color="#10b981" delay={0.7}>
          <ArchNode label="Complexity Analyzer" color="#10b981" delay={0.8} />
          <ArchNode label="Smart Model Router" color="#10b981" delay={0.85} highlight />
          <ArchNode label="Circuit Breaker" color="#10b981" delay={0.9} />
        </ArchRow>
        ...
        {/* Row 4: Egress */}
        <ArchRow label="Egress" color="#f59e0b" delay={1.0}>
          <ArchNode label="Injection Detector" color="#f59e0b" delay={1.1} />
          <ArchNode label="Response Validator" color="#f59e0b" delay={1.15} />
          <ArchNode label="Loop Detector" color="#f59e0b" delay={1.2} highlight />
        </ArchRow>
        ...
        {/* Row 5: Observability */}
        <ArchRow label="Observe" color="#ef4444" delay={1.3}>
          <ArchNode label="OpenTelemetry Traces" color="#ef4444" delay={1.4} mono />
          <ArchNode label="Prometheus Metrics" color="#ef4444" delay={1.45} mono />
          <ArchNode label="Audit Trail" color="#ef4444" delay={1.5} />
          <ArchNode label="Slack Alerts" color="#ef4444" delay={1.55} />
        </ArchRow>
      </div>
    </div>
  );
...
function GatewayContent({ section }: { section: string }) {
  switch (section) {
    case 'overview':
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-600 leading-relaxed">
            A centralized enterprise proxy that sits between AI clients (Kiro, IDEs, LangGraph apps, MCP agents) and LLM providers (AWS Bedrock, OpenAI, Ollama) — providing token cost control, security enforcement, smart model routing, and unified observability for AI operations at scale.
          </p>
          <p className="text-sm text-gray-600 leading-relaxed">
            Solves the critical governance gap that emerges when enterprise teams adopt AI coding assistants and agentic workflows without centralized control over cost, security, and compliance.
          </p>
          <div className="rounded-lg bg-gray-50 border border-gray-100 p-3">
            <p className="text-xs font-semibold text-dark-900 mb-1">Core Question</p>
            <p className="text-sm text-gray-600 italic">"How do we let teams use expensive AI models freely while preventing runaway costs, data leakage, and security breaches — without changing how developers work?"</p>
          </div>
          <div className="flex flex-wrap gap-4 text-center">
            {[
              { value: '7', label: 'Problems Solved' },
              { value: '40-60%', label: 'Cost Savings' },
              { value: '~50ms', label: 'Added Latency' },
              { value: '10', label: 'Design Patterns' },
            ].map(s => (
    ...
    case 'problem':
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-600 leading-relaxed">
            When teams use LangChain, LangGraph, and MCP tools in production without governance, seven critical failures emerge — costing enterprises thousands in wasted tokens, creating security vulnerabilities, and leaving zero visibility into AI operations.
          </p>
          <ul className="space-y-2">
            {[
              { title: '$200 Silent Loops', desc: 'LangGraph agents loop silently calling the same MCP tool for 20+ minutes, burning hundreds of dollars before anyone notices' },
              { title: 'Prompt Injection via Tools', desc: 'Malicious content in database rows hijacks agents through MCP tool responses — LLMs treat tool output as trusted instructions' },
              { title: 'Token Cost Explosion', desc: 'Developers use Claude Opus for simple formatting tasks. No budgets, no visibility, no accountability for spend' },
              { title: 'Data Leakage', desc: 'API keys, passwords, and proprietary code sent to external LLM providers without any scanning or controls' },
              { title: 'Observability Black Hole', desc: 'MCP tool calls are invisible to LangSmith. No unified tracing across LLM + tool + agent interactions' },
            ].map(item => (
          ...
          <div className="rounded-lg bg-red-50 border border-red-100 p-3">
            <p className="text-xs font-semibold text-red-700 mb-1">Market Gap</p>
            <p className="text-xs text-red-600">Existing API gateways (Kong, Apigee) don't understand tokens or AI loops. LangSmith/LangFuse observe but don't enforce. AWS Bedrock Guardrails only work for Bedrock. No single platform provides governance + enforcement + observability across all AI providers and tools.</p>
          </div>
        </div>
      );
    case 'solution':
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-600 leading-relaxed">A transparent proxy architecture with six governance modules — developers change one config line to route through the gateway:</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              { num: '01', title: 'Smart Model Router', desc: 'Analyzes request complexity and routes simple tasks to cheap models (Haiku) automatically. Saves 40-60% without developer effort.' },
              { num: '02', title: 'Loop Detector & Circuit Breaker', desc: 'Redis tracks tool call patterns per session. Same tool >5 times in 60s → session killed. Provider failures → circuit opens.' },
              { num: '03', title: 'Security Scanner (DLP + Injection)', desc: 'Outgoing: strips secrets/PII before hitting provider. Incoming: scans tool responses for prompt injection patterns.' },
              { num: '04', title: 'Token Budget Engine', desc: 'Per-user/team daily & monthly limits. Configurable actions: BLOCK, DOWNGRADE to cheaper model, or ALERT_ONLY.' },
              { num: '05', title: 'Policy Engine', desc: 'Configurable rules: model access control, tool allowlists, content filters, cost ceilings. Dynamic add/remove via admin API.' },
              { num: '06', title: 'Unified Observability', desc: 'Single trace_id across LLM + MCP + agent. Prometheus metrics, Grafana dashboards, Slack alerts for anomalies.' },
            ].map(c => (
    ...
    case 'architecture':
      return (
        <div className="space-y-4">
          <p className="text-sm text-gray-600 leading-relaxed">
            Chain of Responsibility pipeline — each filter can allow, block, or modify requests. Non-blocking reactive architecture handles thousands of concurrent AI requests. Redis for shared state across HA instances.
          </p>
          <GatewayArchDiagram />
          <div className="space-y-1">
            <p className="text-xs font-semibold text-dark-900">Design Patterns</p>
            <div className="flex flex-wrap gap-1.5">
              {['Chain of Responsibility', 'Strategy', 'Circuit Breaker', 'Proxy', 'Adapter', 'Event Sourcing', 'Sliding Window', 'Object Pool', 'Facade', 'Template Method'].map(s => (
    ...
    case 'impact':
      return (
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              { title: '$200 → $2 per incident', desc: 'Loop detection kills runaway agents after 5 iterations instead of 200. 99% cost reduction on failure scenarios.' },
              { title: '40-60% Token Cost Savings', desc: 'Smart router automatically downgrades simple tasks to economy models without developer awareness or effort.' },
              { title: 'Zero Data Leakage', desc: 'DLP scanner catches AWS keys, passwords, PII before they leave the network. Regex-based, configurable, zero latency.' },
              { title: 'Full Audit Compliance', desc: 'Every request logged immutably: who, what model, how many tokens, cost, timestamp. Auditor-ready from day one.' },
            ].map(i => (
          ...
          <div className="rounded-lg bg-green-50 border border-green-100 p-3">
            ...
            <p className="text-[11px] text-green-600">Addresses a market gap that existing tools (LangSmith, Kong, Bedrock Guardrails) don't fill. Combines governance enforcement + security scanning + cost optimization + observability in a single platform. Fully configurable — switch models, MCP servers, and policies via YAML without code changes. Production-ready with HA, circuit breakers, and reactive non-blocking architecture.</p>
          </div>
        </div>
      );
    default:
...
export function CaseStudiesSection() {
  ...
  const [gatewayActiveSection, setGatewayActiveSection] = useState('overview');
        ...
        <AnimatedSection variant="fadeUp" delay={0.1}>
          <div className="relative rounded-[20px] overflow-hidden mb-6">
            ...
            <div className="absolute inset-0 rounded-[20px] p-[1px]"
              style={{ background: 'linear-gradient(135deg, rgba(239,68,68,0.5), rgba(245,158,11,0.4), rgba(16,185,129,0.4))' }}>
              <div className="w-full h-full rounded-[20px] bg-white" />
            </div>

            <div className="relative">
              {/* Top accent bar */}
              <div className="h-[3px]" style={{ background: 'linear-gradient(90deg, #ef4444, #f59e0b, #10b981)' }} />

              {/* Card header */}
              <button
                onClick={() => setIsGatewayExpanded(prev => !prev)}
                ...
                <div className="p-4 sm:p-5 flex flex-col sm:flex-row gap-4 sm:gap-5 items-start sm:items-center">
                  ...
                  <div className="flex sm:flex-col items-center gap-3 sm:gap-1.5 shrink-0">
                    <div className="w-11 h-11 rounded-xl flex items-center justify-center"
                      style={{ background: 'linear-gradient(135deg, rgba(239,68,68,0.12), rgba(245,158,11,0.08))' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        href="https://github.com/jayanthkumarnandimandalam/AI-Gateway-Governance-Platform"
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={(e) => e.stopPropagation()}
                        className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-gray-50 border border-gray-200/80 text-gray-500 hover:text-red-500 hover:border-red-200 transition-colors text-[11px] font-medium"
                    ...
                    <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                      Enterprise AI operations proxy solving runaway token costs, prompt injection, data leakage, and observability gaps. Smart model routing saves 40-60% on token spend. Configurable governance for LLM providers and MCP tool servers.
                    </p>
                    {/* Tech tags */}
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      {['Java 21', 'Spring WebFlux', 'Spring Cloud Gateway', 'Redis', 'PostgreSQL', 'Prometheus', 'OpenTelemetry', 'Docker', 'MCP'].map(t => (
                  ...
                  {/* Right: expand arrow */}
                  <motion.div
                    className="w-8 h-8 rounded-lg bg-gray-50 border border-gray-200/80 flex items-center justify-center text-gray-400 hover:text-red-500 hover:border-red-200 transition-colors shrink-0"
                    animate={{ rotate: isGatewayExpanded ? 180 : 0 }}
                      ...
                      <div className="flex flex-col md:flex-row">
                        ...
                        <div className="md:w-44 shrink-0 border-b md:border-b-0 md:border-r border-gray-100 p-3 md:p-4">
                          ...
                          <ul className="hidden md:flex flex-col gap-0.5">
                            {GATEWAY_SECTIONS.map((s, i) => {
                              const isActive = s.id === gatewayActiveSection;
                              return (
                                <li key={s.id}>
                                  <button
                                    type="button"
                                    onClick={() => setGatewayActiveSection(s.id)}
                                    className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                                      isActive ? 'text-red-600 bg-red-50 border-l-2 border-red-500' : 'text-gray-500 hover:text-dark-900 hover:bg-gray-50'
                                    }`}
                                  >
                                ...
                                <li key={s.id}>
                                  <button
                                    ...
                                    className={`whitespace-nowrap px-2.5 py-1 rounded-full text-[10px] font-medium transition-colors ${
                                      isActive ? 'text-white bg-red-500' : 'text-gray-500 bg-gray-50'
                                    }`}
                                  >
                                    {s.title}
                                  </button>
                            ...
                            <AnimatePresence mode="wait">
                              ...
                              >
                                <h3 className="font-heading font-bold text-dark-900 text-lg mb-3 pb-2 border-b border-gray-100">
                                  {GATEWAY_SECTIONS.find(s => s.id === gatewayActiveSection)?.title}
                                </h3>
                                <GatewayContent section={gatewayActiveSection} />
                              </motion.div>
                            </AnimatePresence>
                          </div>
